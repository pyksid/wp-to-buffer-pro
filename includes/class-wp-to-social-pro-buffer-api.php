...
class WP_To_Social_Pro_Buffer_API {
	...
	public function profiles( $force = false, $transient_expiration_time = 43200 ) {
		...
		$profiles = get_transient( $this->base->plugin->name . '_buffer_api_profiles' );
					...
					if ( $this->is_local_host() ) {
						$message = array(
							sprintf(
								...
								__( 'Buffer could not fetch the image %s because your site is running on a local host and not web accessible. Please run the Plugin on a publicly accessible domain.', 'wp-to-social-pro' ),
						...
						$message = array(
							sprintf(
								...
								__( 'Buffer could not fetch the image %1$s.  Install %2$s to automatically remove spaces, accented or special characters in image filenames. If the issue persists, please work with Buffer to resolve.', 'wp-to-social-pro' ),
								( isset( $params['media']['picture'] ) ? $params['media']['picture'] : '' ),
